...
function getAPI($id, $order, $continuationToken)
{
    ...
    if (isset($_GET['hashTag'])) {
        ...
        $rawData = '{"context":{"client":{"clientName":"WEB","clientVersion":"' . CLIENT_VERSION . '"}},"' . ($continuationTokenProvided ? 'continuation":"' . $continuationToken : 'browseId":"' . $id . '","params":"' . $orderBase64) . '"}';
        $opts = [
            "http" => [
                "method" => "POST",
                "header" => "Content-Type: application/json",
